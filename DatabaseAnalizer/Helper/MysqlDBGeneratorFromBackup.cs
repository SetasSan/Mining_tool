...
namespace DatabaseAnalizer.Helper
{
    public class MysqlDBGeneratorFromBackup
    {
        ...
        public void createDbFormBackup(out string dbName)
        {
            ...
            List<string> sDbs1 = (new DBCommandGenerator(connection)).getStringList("select SCHEMA_NAME from information_schema.SCHEMATA");
            cmd.Connection = connection;
            connection.Open();
            bcp.ImportFromFile(this.model.dbBackupPath);
            connection.Close();
            List<string> sDbs2 = (new DBCommandGenerator(connection)).getStringList("select SCHEMA_NAME from information_schema.SCHEMATA");
            connection.Close();
